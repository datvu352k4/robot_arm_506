 ...
 * The movement can be both clo ckwise or counterclockwise based on the values assigned to
...
void reach_goal_shoulder(Servo& motor, Servo& motor2, int goal){
  
  if(goal>=motor.read()){
    
    // goes from the start point degrees to the end point degrees
    for (int pos = motor.read(); pos <= goal; pos += 1) { 
      int pos1=map(pos,0,180,-90,90);
      motor.write(90+pos1); 
      motor2.write(90-pos1);     
      delay(5);                       
    }
  } else{
    // goes from the end point degrees to the start point degrees
...
void setup() {
  ...
  shoulder2.attach(SERVO_SHOULDER2_PIN);
  ...
  shoulder2.write(SHOULDER2_START);
...
void loop() {
  if (Serial.available())
  {
    ...
    else if(chr == ',')
    {
      ...
      {
        reach_goal_shoulder(shoulder, shoulder2, val);
      }
      else if(idx == 2)
      {
